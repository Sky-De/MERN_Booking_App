...
const SearchForm = () => {
  const {destination, date, options, dispatch} = useContext(SearchContext);
  ...
  return (
    <form className='form'>
        ...
        <div className="form__inputControl">
            ...
            <span>{((formData.date[0].endDate - formData.date[0].startDate) / (1000 * 60 * 60 * 24)) + 1} {((formData.date[0].endDate - formData.date[0].startDate) / (1000 * 60 * 60 * 24)) + 1 > 1 ? "days" : "day"}</span>
            {openDate && <DateRange
